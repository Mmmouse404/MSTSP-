/*
@author 黄鸿展 202230441138
@date 2024-07-04

以下代码为改进的NMA算法,仅供《智能算法导论》课程大作业使用
原算法版权声明如下
*/
...
int GeneticAlgorithm::localSearch(Chromosome& chrom)
{
		...
		for (int i = 0; i < city_num - 2; i++)
		{
			for (int j = i + 1; j < city_num - 1; j++)
			{
				prove = dist[chrom.path[(i - 1 + city_num) % city_num]][chrom.path[i]] + dist[chrom.path[j]][chrom.path[(j + 1) % city_num]]
					- dist[chrom.path[(i - 1 + city_num) % city_num]][chrom.path[j]] - dist[chrom.path[i]][chrom.path[(j + 1) % city_num]];
				count += 4;
	...
	else if (local_search_operator == "or-opt")
	{
		int count = 0;
		distType pre_dist = chrom.length;
		distType best_improve = 0;
		distType prove;
		int best_i = 0, best_j = 0; // 初始化 best_i, best_j

		for (int i = 0; i < city_num - 1; i++)
		{
			for (int j = 0; j < city_num; j++)
			{
				if (j >= i && j <= i + 1) continue; // 跳过原位置

				// 计算移动后的路径长度
				distType new_length = 0;
				for (int k = 0; k < city_num; k++)
				{
					int from = chrom.path[k];
					int to = chrom.path[(k + 1) % city_num];
					if (k == i)
					{
						new_length += dist[chrom.path[i]][chrom.path[i + 1]];
						k++;
					}
					else if (k + 1 == j)
					{
						new_length += dist[chrom.path[i + 1]][chrom.path[j + 1]];
					}
					else
					{
						new_length += dist[from][to];
					}
				}

				prove = pre_dist - new_length;
				count += 6;
				if (prove >= best_improve)
				{
					best_improve = prove;
		...
		{
			// 移动两个城市
			vector<int> segment(chrom.path.begin() + best_i, chrom.path.begin() + best_i + 2);
			chrom.path.erase(chrom.path.begin() + best_i, chrom.path.begin() + best_i + 2);
			chrom.path.insert(chrom.path.begin() + best_j, segment.begin(), segment.end());

			// 更新路径长度
			chrom.length = 0;
			for (int m = 0; m < city_num; m++)
			{
				chrom.length += dist[chrom.path[m]][chrom.path[(m + 1) % city_num]];
			}
		}
		return count;
	}
...
{
	// 计算种群的平均适应值
	distType total_fitness = 0;
	for (int i = 0; i < NP; i++)
	{
		total_fitness += chroms[i].length;
	}
	distType avg_fitness = total_fitness / NP;
	int  p1, p2, p;
	for (int i = 0; i < NP; i += 2)
	{
		double p_i = crossover_ratio; // 默认交叉概率
		if (chroms[i].length < avg_fitness)
		{
			p_i = 0.9 - 0.3 * (1 - static_cast<double>(chroms[i].length) / avg_fitness);
		}
		else if (chroms[i].length > avg_fitness)
		{
			p_i = 0.9;
		}
		if (rnddouble(0.0, 1.0) >= p_i)
			continue;
...
void GeneticAlgorithm::mutation()
{
	// 计算种群的平均适应度和最大适应度
	distType total_fitness = 0;
	distType max_fitness = 0;
	for (int i = 0; i < NP; i++)
	{
		total_fitness += chroms[i].length;
		if (chroms[i].length > max_fitness)
		{
			max_fitness = chroms[i].length;
		}
	}
	distType avg_fitness = total_fitness / NP;
	...
	for (int i = 0; i < NP; i++)
	{
		double p_i = mutation_ratio; // 默认变异概率
		if (chroms[i].length < avg_fitness)
		{
			p_i = mutation_ratio - 0.05 * (static_cast<double>(chroms[i].length) - avg_fitness) / (max_fitness - avg_fitness);
		}
...
void GeneticAlgorithm::00(GeneticAlgorithm& Parent, const string& s)
{
	int p1, p2;
	if (s == "tournament")
	{
		int tournament_size = 3; // 锦标赛大小，可以根据需要调整
		for (int i = 0; i < NP; i++)
		{
			int best_index = rndint(0, NP - 1);
			for (int j = 1; j < tournament_size; j++)
			{
				int candidate_index = rndint(0, NP - 1);
				if (Parent.chroms[candidate_index].length < Parent.chroms[best_index].length)
				{
					best_index = candidate_index;
				}
			}
			chroms[i] = Parent.chroms[best_index];
		}
	}
	else if (s == "roulette")
	{
